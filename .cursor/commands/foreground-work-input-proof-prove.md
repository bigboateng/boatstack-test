---
name: foreground-work-input-proof-prove
description: "Run repository Flow entry prove to target approved-plan. Use only when the user explicitly selects this repository Flow entry."
---

# Foreground Work Input Proof Prove

Run the repository-owned Flow "foreground-work-input-proof" entry "prove" until its marked target "approved-plan" is reached.
...
Start with `boatstack flow run --repo . --flow foreground-work-input-proof --entry prove --repository-authority --host cursor --format json`.
...
`boatstack flow authorize --repo . --flow foreground-work-input-proof --entry prove --run-id <run-id> --request-fingerprint <fingerprint> --human-identity-provider-fingerprint <provider-fingerprint> --human <actor> --host cursor`

After authorization, use `boatstack flow run --repo . --flow foreground-work-input-proof --entry prove --run-id <run-id> --repository-authority --host cursor --format json`.
...
`boatstack flow work input-required --repo . --flow foreground-work-input-proof --entry prove --run-id <run-id> --work-id <work-id> --prompt <question> --host cursor --format json`
...
`boatstack reconcile-update --repo . --flow foreground-work-input-proof --entry prove --run-id <run-id> --accept-program-change --human <actor> --host cursor --format json`
