---
name: work-package-proof-accept
description: "Run repository Flow entry accept to target approved-package. Use only when the user explicitly selects this repository Flow entry."
---

# Work Package Proof Accept

Run the repository-owned Flow "work-package-proof" entry "accept" until its marked target "approved-package" is reached.
...
Start with `boatstack flow run --repo . --flow work-package-proof --entry accept --repository-authority --host cursor --format json`.
...
`boatstack flow authorize --repo . --flow work-package-proof --entry accept --run-id <run-id> --request-fingerprint <fingerprint> --human-identity-provider-fingerprint <provider-fingerprint> --human <actor> --host cursor`

After authorization, use `boatstack flow run --repo . --flow work-package-proof --entry accept --run-id <run-id> --repository-authority --host cursor --format json`.
...
`boatstack flow work input-required --repo . --flow work-package-proof --entry accept --run-id <run-id> --work-id <work-id> --prompt <question> --host cursor --format json`
...
`boatstack reconcile-update --repo . --flow work-package-proof --entry accept --run-id <run-id> --accept-program-change --human <actor> --host cursor --format json`
