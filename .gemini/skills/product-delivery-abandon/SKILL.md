...
Start with `boatstack flow run --repo . --flow product-delivery --entry abandon --repository-authority --host gemini --format json`.
...
`boatstack flow authorize --repo . --flow product-delivery --entry abandon --run-id <run-id> --request-fingerprint <fingerprint> --human-identity-provider-fingerprint <provider-fingerprint> --human <actor> --host gemini`

After authorization, use `boatstack flow run --repo . --flow product-delivery --entry abandon --run-id <run-id> --repository-authority --host gemini --format json`.
